...
| P-Consult | 只解释给定段落与已确认事实 | S1/S3 是；S0/S2/覆盖层/建议卡否 | 建议卡 accept 走既有建档入口 |
...
| S3 | 搜索兜底 | 本地无说明书且开关开 | 无个体化 | 百川搜索 | 标注「未经本库核实」；禁止个体化建议 | 固定引导 | 开关默认关的断言 |
