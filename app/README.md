...
| `OCR_BASE_URL` | 医嘱线 OCR | — | PaddleOCR 自托管服务地址（`POST /ocr`）；缺失 → OCR `AIUnavailableError`（录入转 `OCR_FAILED` 降级草稿） |
| `OCR_PROVIDER` / `OCR_API_KEY` | | — | 备选云端 OCR 占位（ADR#13 定案为 PaddleOCR 自托管，未启用） |
