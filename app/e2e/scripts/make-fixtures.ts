 ...
 * （见 fixtures/README.md），mock OCR 为行级文本转录（qwen3.5-ocr 契约，无字符级坐标/置信度）；故无需真实模型即可冻结 I/O。
...
function main() {
  ...
  for (const [name, sc] of Object.entries(spec.scenarios)) {
    ...
    const pack = {
      ...
      // 仅入口A 跑 OCR；涂黑行已从 OCR 可见文本中剔除（行级文本转录，qwen3.5-ocr 契约）
      runOcr: isRx ? { lines: ocrLines(sc) } : null,
